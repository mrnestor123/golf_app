...
export { alertDialog, confirmDialog, openDialog, Dimmer, Modal, ModalContent, ModalFooter, ModalHeader, openPopup, promptDialog, showSnackbar };
...
function confirmDialog(options = { 'title': '', 'message': '', 'buttonLabels': [], 'then': () => { }, 'multiple': false, 'onSaveAnswer': () => { }, size: 'tiny' }) {
    ...
    m.mount(elem, {
        ...
        view: () => m(Modal, { size: options.size || 'tiny' }, m(ModalHeader, m(H2, options.title || 'Confirma la acción')), m(Div, { padding: '1em' }, m(Text, m.trust(options.message))), m(ModalFooter, m(Button, {
            type: 'positive',
...
function alertDialog(options = {
    ...
    m.mount(elem, {
        ...
        view: () => m(Modal, { size: options.size || 'tiny' }, types[options.type] || options.title ?
            m(ModalHeader, m(Icon, { icon: types[options.type]?.icon, color: types[options.type]?.color }), m(Box, { width: '10px' }), m(H2, { marginTop: 0 }, options.title || types[options.type]?.text)) : null, m(ModalContent, m(Div, { padding: '1em' }, m(Text, m.trust(options.message)))), m(ModalFooter, m(Button, {
            onclick: (e) => {
    ...
    return {
        view: (vnode) => {
            ...
            return m("div", {
                ...
                m(ModalHeader, { justifyContent: 'space-between', borderBottom: '2px solid lightgrey', alignItems: 'center' }, m(H2, { marginBottom: 0 }, vnode.attrs.header), m(SVGIcon, {
                    width: 26, height: 26,
...
function ModalFooter() {
    return {
        view: (vnode) => {
            return m(FlexRow, { borderTop: '2px solid lightgrey', justifyContent: 'end', padding: '1em', gap: '1em' }, vnode.children);
        }
    };
}
function Dimmer() {
    ...
    return {
        view: (vnode) => {
            ...
            return m(Div, { background: inverted ? 'rgba(255,255,255,.85)' : 'rgba(0,0,0,.85)', ...dimmerStyle }, vnode.children);
        }
    };
}
