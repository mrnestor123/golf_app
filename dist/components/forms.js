...
export { FormLabel, Input, TranslationInput, Dropdown, IntegerInput, Switch, InfoTooltip, Checkbox, RadioButtons, HtmlIntegerInput, HtmlDropdown, DateSelector };
    ...
    return {
        view: (vnode) => {
            ...
            return [
                ...
                }, typeof vnode.children?.[0] == 'object' ? null : vnode.children), required
...
function Checkbox() {
    ...
    return {
        view: (vnode) => {
            ...
            return [
                m(FlexRow, { alignItems: "center", flexDirection: vertical ? "column-reverse" : "row", gap: '0.5em' }, m("input", {
                    type: 'checkbox',
                    disabled,
        ...
        view: (vnode) => {
            ...
            return [
                ...
                label
                    ...
                    ] : null, m(Div, { position: 'relative', width: '100%', display: 'flex' }, m(type == 'textarea' ? "textarea" : "input", {
    ...
    return {
        ...
        view: (vnode) => {
            ...
            return m(FlexCol, { width: '100%', }, label ? m(FormLabel, { required: required, info: info }, label) : null, m(FlexRow, m(Input, {
                ...
                oninput: (e) => {
                    if (!e.target.value.length && typeof value == "object")
                        delete data[name][languages[selectedlang]];
                },
                type: type,
                ...
                : languages[selectedlang])), m(FlexRow, { justifyContent: "space-between", alignItems: "center", paddingTop: "3px" }, m(FlexRow, { gap: "1rem", marginRight: "auto" }, languages.map((l, i) => {
...
function Dropdown() {
    return {
        view: (vnode) => {
            ...
            return [
                m(FlexCol, { width: '100%', ...vnode.attrs.style }, label ? m(FormLabel, { info: info, required: required }, label) : null, m("select", {
                    disabled,
                    style: {
                        ...(config.form?.baseStyle),
                        appearance: 'none',
                ...
                }, m("option", { disabled: true, selected: true }, placeholder || "Selecciona una opción"), vnode.children.map((o) => m("option", {
...
function RadioButtons() {
    return {
        view: (vnode) => {
            ...
            return [
                m(FlexCol, { width: '100%', gap: "5px", ...vnode.attrs.style }, label ? m(FormLabel, { info: info, required: required }, label) : null, m(FlexCol, { gap: "10px" }, vnode.children.map((o) => m(Tappable, {
                    style: {
                        display: "flex",
                        gap: "5px",
                        alignItems: "center"
...
function DateSelector() {
    ...
    return {
        view: (vnode) => {
            ...
            return [
                m(FlexCol, { width: '100%' }, m(FormLabel, { required }, label), m(Tappable, {
                    clickout: (e) => {
                        if (!focused)
                            return;
                ...
                m("input", {
                    ...
                    m(SmallText, { style: { position: 'absolute', bottom: '-20px', color: 'grey' } }, 'aaaa/mm/dd') : null, m(Icon, {
                    icon: 'calendar_today',
                    color: 'rgba(34, 36, 38)'
                }))))
            ];
        ...
        view: (vnode) => {
            ...
            return [
                m(FlexCol, { width: '100%' }, m(FormLabel, { required }, label), m(Tappable, {
                    ...
                    onclick: (e) => open = !open
                }, m(FlexRow, { justifyContent: 'space-between', alignItems: 'center', height: '100%' }, m(Text, {
                    maxWidth: '80%',
                    overflow: 'hidden',
                    textOverflow: 'ellipsis',
                    whiteSpace: 'nowrap',
...
function IntegerInput() {
    ...
    return {
        view: (vnode) => {
            ...
            return [
                m(FlexCol, label ? m(FormLabel, { required: required }, label) : null, m("div", {
                    style: {
                        ...(config.form?.baseStyle),
                        ...style
                    }
                }, m(FlexRow, { alignItems: 'center', justifyContent: 'space-between' }, m("div", data && name && data[name] ? data[name] : 0, 
