...
export function LoginPage() {
  ...
  function validateField(fieldName: 'email' | 'password' | 'confirmpassword') {
    if (fieldName == 'email') {
      if (!data.email) {
  ...
  function validateForm() {
    ...
    if (activemode == 'register' && !validateField('confirmpassword')) isValid = false;

    return isValid;
  }

  function renderFieldError(fieldName: 'email' | 'password' | 'confirmpassword') {
    ...
    view: () => {
      ...
      return [
        m(App,
          ...
          m(AppContent, { alignItems: 'center', padding: '1rem', paddingTop: '2rem', gap: '1.5rem', textAlign: 'center' },
            m(Div, {
            ...
            m(FlexCol, {
              ...
              activemode == 'login' ? m(LoginForm) : m(RegisterForm)
            ),
            ...
            try {
              ...
              interval = setInterval(async () => {
                try {
                  ...
                  loading = false;
                  provider = '';
                  error = mapAuthError((e as any)?.message);
                  clearInterval(interval)
                  m.redraw()
                }
              }, 500);
    ...
    return {
      view: (vnode) => {
        ...
        return m(Animate, {
          ...
          m(FlexRow, { alignItems: 'center', justifyContent: 'center', gap: '2rem' },
            m(LucideIcon, { icon: 'alert-circle', width: '20', height: '20', style: { color: 'white' } }),
            m(Text, { color: 'white', flex: 1, width: '80%' }, text)
          )
  ...
  function Switch() {
    ...
    return {
      view: () => {
        return [
          m(FlexRow, { padding: '0.5em', background: config.colors.lightgrey, borderRadius: config.borderRadius, width: '80%', margin: '0 auto' },
  ...
  function LoginForm() {
    ...
    async function login() {
      ...
      try {
        ...
        loading = false;
        provider = '';
        error = mapAuthError((signInError as any)?.message);
        m.redraw()
      }
    }
  }
    ...
    view: () => {
      ...
      return [
        m("div", {
          ...
          popupLoading
            ? m(FlexCol, { alignItems: 'center', gap: '0.8rem' },
              m(Spinner),
              m(Text, 'Signing you in...')
            )
            : m(FlexCol, { alignItems: 'center', gap: '0.8rem', padding: '1rem', textAlign: 'center' },
              m(LucideIcon, { icon: 'alert-circle' }),
              m(Text, popupError || 'Sign in failed. Please try again.')
            )
        )
      ]
    }
